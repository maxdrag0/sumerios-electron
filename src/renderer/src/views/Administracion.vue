<template>
  <div class="container-view">
    <h1>Vista de la Administración</h1>
      ...
      <div class="container">
        ...
        <div class="container-datos">
                  ...
                  <div v-else>
                    <h3 v-if="administracion.nombre">Nombre: <p>{{ administracion.nombre }}</p></h3>
                    <h3 v-if="administracion.direccion">Direccion: <p>{{ administracion.direccion }}</p></h3>
                    <h3 v-if="administracion.telefono">Telefono: <p>{{ administracion.telefono }}</p></h3>
                    <h3 v-if="administracion.cuit">Cuit: <p>{{ administracion.cuit }}</p></h3>
                    <h3 v-if="administracion.mail">Mail: <p>{{ administracion.mail }}</p></h3>
                    <h3 v-if="administracion.nombreAdministrador">Administrador: <p>{{ administrador.nombre }}</p></h3>
                    <h3 v-if="administracion.matriculaAdministrador">Matricula: <p>{{ administracion.matriculaAdministrador }}</p></h3>
...
const updateAdm = async () => {
  try {
    ...
    const response = await axios.put(`${API_ADM}/${adminStore.administracionData.idAdm}`, administracionEdit.value);
    console.log('Administración actualizada:', response.data);
    alert('Administración '+administracionEdit.value.nombre+' actualizada con éxito.');
    edicion.value = false;
    unsavedChanges.value = false;
    cargarAdm(adminStore.administracionData.idAdm);
  } catch (error) {
    console.error('Error actualizando la administración:', error);
    alert('Hubo un error al actualizar la administración.');
  }
};
